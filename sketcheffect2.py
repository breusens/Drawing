        ...
        if not beeld is None:
            ...
            for rb in range(rows//NBlock+1):
                for cb in range(cols//NBlock+1):
                    ...
                    img_back1=np.exp(img_back1)/(1+np.exp(img_back1))*255
                    img_back2=np.exp(img_back2)/(1+np.exp(img_back2))*255
                    img_back3=np.exp(img_back3)/(1+np.exp(img_back3))*255
